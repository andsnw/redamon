...
Two **append-only** tables record security-relevant events. `AuditLog` (`audit_log`) stores `actorId`, `action`, `targetType`, `targetId`, before/after JSON, and a `source` (api/ui/admin/system), indexed for lookup by target and by actor. `ActAsAudit` (`act_as_audit`) records admin impersonation with `event` = `start`/`end` as **separate inserts** (an end is never an in-place update, preserving the trail). Actor columns are nullable so existing rows need no backfill. Audited `AuditLog` actions: authentication (`auth.login.success`, `auth.login.failure`, `auth.login.lockout`, `auth.logout`); every MCP write and a denied MCP call (`mcp.start_recon`, `mcp.start_recon.refused`, `mcp.stop_recon`, `mcp.update_recon_settings`, `mcp.create_project`, `mcp.attach_authorization`, `mcp.set_finding_verdict`, `mcp.auth.denied`); triage runs (`triage.start`, `triage.finish`); and Mute Rules, which decide what the operator and the agent can see (`node_filters.saved`, naming a loaded preset, `node_filters.mode_changed`, `node_filters.applied`, `node_filters.apply_finished`, `node_filters.armed`, `node_filters.disarmed`, `node_filters.exemptions_cleared`, and `muted_nodes.unmuted` with what had muted each finding). Impersonation (act-as start and end) goes to `ActAsAudit`. Mute Rules rows record the real actor behind an act-as session. Writes are best-effort so a logging failure never breaks a request, and never record passwords or hashes. In the deploy posture the real client IP is trustworthy because `TRUST_PROXY` marks the nginx-supplied `X-Forwarded-For` authoritative.
