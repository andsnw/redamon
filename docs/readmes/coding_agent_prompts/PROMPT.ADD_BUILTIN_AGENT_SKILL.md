...
| 6b | **Inbound MCP settings allowlist** | [webapp/src/lib/reconSettingsAllowlist.generated.ts](../../../webapp/src/lib/reconSettingsAllowlist.generated.ts) | Only if Layer 6 added `Project` columns. Each one goes in the **DENY** table (`'attack'`, `'egress'` or `'wordlist'` per the shipped skills). A coverage test reading `Prisma.ProjectScalarFieldEnum` fails until every column is classified, so this layer announces itself — but it announces itself in the webapp suite, not the agent one, which is why it gets missed. |
...
- [ ] **Every per-skill tunable that became a `Project` column is classified in [reconSettingsAllowlist.generated.ts](../../../webapp/src/lib/reconSettingsAllowlist.generated.ts)** (Layer 6b). The inbound MCP server writes settings through a positive allowlist, and a coverage test walking `Prisma.ProjectScalarFieldEnum` is red until every new column sits in ALLOW or DENY. Follow the shipped skills: `sqliLevel: 'attack'`, `ssrfOobCallbackEnabled: 'egress'`, `ssrfOobProvider: 'egress'`, `rceAggressivePayloads: 'wordlist'`. **DENY is the right answer for every attack-skill tunable** — these change what the agent does to a live target, which is not something an external token may reach. A skill with no Prisma-backed tunables needs nothing: `attackSkillConfig` itself is already classified.
