...
- **An external triage assistant's judgement now persists.** `set_finding_verdict` marks a finding confirmed, likely noise, or back to unreviewed, behind a new **`triage:write`** permission. Without it an agent's conclusions were recomputed from scratch by the next nightly run.

  It records the verdict as the operator's own — the token carries their authority — rather than inventing a third provenance value, and that is a deliberate correction of the obvious design. `triage_source` is a closed two-value set that four separate behaviours branch on: a third value would make the finding **prune-eligible**, so the next scan deletes it instead of marking it superseded; it would let a later AI run overwrite the verdict; it would stop `likely_noise` meaning false-positive; and the board would render it as "Not reviewed". The channel is recorded on its own property instead, alongside the actor, so provenance stays auditable without changing what the verdict *means*.

  It refuses while a triage run is in progress, and fails closed if it cannot tell. A run's measurement step is unconditional and a verdict does not touch `updated_at`, so a verdict written mid-run would be silently re-filed into a board section contradicting it — rare when a person clicks, routine for an unattended agent. A verdict is also now **audited**, on both the agent and the MCP side; previously a durable decision that suppresses future AI review of a finding was recorded nowhere at all.

  **It cannot mute or unmute**, in either direction. Muting is the one action that hides a finding from every other read on this surface, and "this is a false positive, mute it" is an entirely plausible instruction to find embedded in scanner output. Unmuting is withheld for a subtler reason: the triage subsystem's stated invariant is that only a person mutes, and that is exactly what bounds a prompt injection to *mislabelling a verdict a human can overrule*.
...
- **Nine defects found by reviewing the new MCP tools as failure modes rather than as code.** Every one produced a wrong answer rather than an error, and each now has a test that fails against the old behaviour.

  **Two silently shrank a security answer.** `list_exploit_paths` and `get_blast_radius` filtered superseded exploit records with a row-level predicate instead of an inner join condition, so a CVE whose only exploit record had been superseded vanished from the ranking entirely, taking its CVSS and sometimes its whole technology with it. `list_findings` applied a severity or section filter to the highest-ranked 2,000 findings and reported the matches as a complete total, so a project with more than that could answer "four criticals" when there were forty; the count is now explicitly a floor when the window was full.

  **One did the wrong thing on request.** `compare_scan_versions` accepted `from: "current"`, documented it, and then silently substituted a stored version, comparing a pair the caller never asked for. Comparing the live graph with itself is now refused instead of captured twice.

  **One crossed a tenant boundary.** `queue_recon` and `compare_scan_versions` charged their per-project rate budget before checking ownership. Because that budget is shared by every token, anyone holding a project id could exhaust another tenant's scan-start window, and its owner was then refused with a message blaming their own token. Ownership is now proven first, as `start_recon` already did.

  **Three were the "never report a dependency failure as an empty result" rule not actually applied.** A 200 response whose payload shape had drifted was read as "nothing is running" by the activity helper and as "zero runs" by `get_scan_status`, which would have let `graph_summary` report a settled graph during a live scan. Both now report unknown. `list_muted_findings` capped what it returned but not what it fetched, pulling every suppressed finding across the wire on every call; the cap now travels with the request and a full window is reported as a floor.

  **Two were operability.** Paging past the 2,000-finding window returned empty pages while still flagging "there is more", so an agent walking a large project looped; it is now refused with the reason. And a deploy that rebuilds the webapp without the agent silently lost both the graph concurrency ceiling and verdict provenance, because the older agent ignores fields it does not know and still answers 200 — the agent now acknowledges the gate and the webapp logs an actionable line naming the rebuild command when it does not.

  `get_project_activity` also moves from the read bucket to the question bucket: one call costs up to eight requests to the scan orchestrator that every project on the host shares, and telling the model not to poll is advice to an untrusted caller rather than a control.

- **`kali_exec` could not be switched on at all on a deployed host.** `MCP_KALI_EXEC_ENABLED` and `MCP_RATE_EXEC_PER_MIN` were wired into the compose file but never seeded into the deployed application's `.env`, so compose interpolated them to empty and fell back to its own `false` default. An operator who set the switch in their deploy `.env` got no error and no effect — the same "reachable from nowhere" bug the inbound server's other keys were fixed for, repeated for the two the sandbox feature added. The repo's own env-chain test was already failing on both; it now passes, and it catches the next one.
